...
codes = {'A':'!', 'B':'@','C':'#','D':'$','E':'%','F':'^','G':'&','H':'*','I':'K','J':'_',
        ...
        'y':'x','z':'v'}
#Reads the contents of text.txt and encrypts it using my dictionary then sends it to the
# encrpted.txt
...
# Reads the encrpted file then decrypts it using my dictionary 
def decrypt_file(input_file_name, codes):
    try:
        ...
        print(f"Error: Input file '{input_file_name}' not found.")
        return
# Reverses codes to character for decryption
    reverse_codes = {v: k for k, v in codes.items()}
    decrypted_text = ''
    for char in encrypted_text:
        if char in reverse_codes:
            decrypted_text += reverse_codes[char]
# Keep the characters that aren't in the dictionary the same
        else:
            decrypted_text += char
